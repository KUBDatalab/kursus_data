...
## Udvikling i fyldt (er der ikke nogen der kan hjælpe med et bedre ord?)
...
meta_data %>%
  ...
  geom_col() +
  ggtitle("udviklingen i hvor fyldte kurserne er pr måned")
```
...
## Graden af "fyldt" pr. kategori pr år.
```{r}
meta_data %>% 
  ...
  pivot_wider(names_from = år,
              ...
              values_fill = 0) %>% 
  gt() %>% 
  tab_header("Hvor mange af pladserne er fyldt - fordelt på kategori og år") %>% 
  tab_footnote("Bemærk at brugen af kategorier i kalendersystemet er relativt ujævn, og mange kategorier derfor er identificeret automatisk. En andel på 0 indikerer alle steder at der ikke har været kursusaktivitet under den pågældende kategori for det pågældende år.")
...
meta_data %>% 
  ...
  geom_point() +
  geom_line() +
  ggtitle("hvor fyldte er kurserne over tid - fordelt på kategorier")
```
...
grupperet_kategori_fyldt_pr_år  %>% 
  ...
  geom_line() +
  facet_wrap(~kategori) +
  ggtitle("Hvor fyldte er kurserne over tid, fordelt på kategorier - sprogene")
```
...
grupperet_kategori_fyldt_pr_år  %>% 
  ...
  geom_line() +
  facet_wrap(~kategori) +
  ggtitle("Hvor fyldte er kurserne over tid, fordelt på kategorier - programmerne")
```
...
grupperet_kategori_fyldt_pr_år  %>% 
  ...
  geom_line() +
  facet_wrap(~kategori) +
  ggtitle("Hvor fyldte er kurserne over tid, fordelt på kategorier - teknikkerne")
```
...
grupperet_kategori_fyldt_pr_år  %>% 
  ...
  geom_line() +
  facet_wrap(~kategori) +
  ggtitle("Hvor fyldte er kurserne over tid, fordelt på kategorier - søjlerne")
```
